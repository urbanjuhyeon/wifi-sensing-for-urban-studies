...
The [urban-sensing-raspi](https://github.com/jhyeonpark/urban-sensing-raspi) GitHub repository hosts the scripts needed to set up the sensing service on a Raspberry Pi. Here's a brief overview of the included files: 

- `install.sh`: This shell script automates the installation process for the Urban Sensing Service. It installs necessary system packages and Python libraries, sets up the Bluetooth logger, and handles other initial setup tasks. 

- `configure.sh`: This shell script handles the configuration of the Wi-Fi Sensing Service. It sets up udev rules to ensure consistent network interface naming and creates a systemd service to run the sensing service automatically at boot. 

- `start.py`: In the 'code/default', this Python script is the main program for the Sensing Service. As a default, it configures WiFi monitoring on three different channels, collects WiFi packet data, parses the data, and stores relevant information in a SQLite database. It also collects Bluetooth data, too.

Once you have completed the tests and verified that everything is functioning as expected, navigate back to the home directory. This can be done by executing the following command:

```bash
cd /home/pi
```

To download these files, you can clone the repository using the following command:
...
After downloading the necessary scripts, you'll need to run both the `install.sh` and `configure.sh` scripts to set up your urban sensing environment.

Start by navigating to the directory that contains the scripts. If you aren't already there, you can get there with the following command:

```bash
cd urban-sensing-raspi
```

Next, run the `install.sh` script. This script installs all the necessary system packages and Python libraries, sets up the Bluetooth logger, and handles other initial setup tasks. Execute it with the following command:

```bash
bash install.sh
```

Once the installation script completes, proceed with the `configure.sh` script. This script sets up udev rules for consistent network interface naming and creates a systemd service to run the sensing service automatically at boot. Run it with this command:

```bash
bash configure.sh
```

Each script will provide progress messages in the terminal, allowing you to monitor the process and troubleshoot any issues that may arise. After successfully running both scripts, your Raspberry Pi is now ready to start sensing!

#### Temporarily Stopping the Sensing Service

From now, your Raspberry Pi will run `start.py` at boot time to begin the sensing operations.

If you wish to perform other tasks (like testing, moving files, etc.) without triggering the sensing script, you'll need to stop the service temporarily by executing the following command in the terminal:

```bash
systemctl stop sensing.service
```

> `start.py` contains a `time.sleep` function, introducing a 30-second pause after startup to stabilize the system. If the sensing service is manually stopped within this window using `systemctl stop sensing.service`, the sensing operations and other processes in `start.py` won't initiate.

Remember to start the service again when you're ready to resume sensing operations.

### Step 3. Testing 

Navigate back to the home by executing the following command:

```bash
cd /home/pi
```

When you run start.py and press `ctrl+C` to stop the code after 1 minute. You could check these: 
